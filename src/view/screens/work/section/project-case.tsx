...
const ProjectCase = (props: { project: string }) => {
  const { projectOverview, projectOverview2, challenge, challengeSVG, solution } =
    workCaseData[props.project]
  return (
    <>
      <section className={Styles.caseStudy}>
        ...
        {solution
          ? solution.data.map((section) => (
              <section
                key={`${section?.title} ${section?.mainTitle}`}
                className={section.greyBG ? Styles.greyBackground : undefined}
              >
                {section.mainTitle ? <h3>{section.mainTitle}</h3> : <h4>{section.title}</h4>}
                {section.para ? section.para.map((para) => <p key={para}>{para}</p>) : null}
                ...
                {section.list
                  ? section.list.map((list) => (
                      <Fragment key={section.list?.indexOf(list)}>
                              ...
                              <Fragment key={img.src}>
                                <LazyLoad once height={200}>
                                  <img
                                    ...
                                    key={img.src}
                                    width="100%"
                                    alt={img.caption ? img.caption : img.src}
                                  />
                                </LazyLoad>
