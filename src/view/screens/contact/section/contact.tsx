...
import ResponsePopupModal from './response-popup-modal'
...
const SectionContact = () => {
  ...
  function copyPhone() {
    void navigator.clipboard.writeText(copyRef.current?.value || '')
    setCopyText({
      copied: true,
      text: copyRef.current?.value || '',
    })
  }

  const contactChangeHandler: ChangeEventHandler<HTMLInputElement | HTMLTextAreaElement> = (e) => {
    setContact({
      ...contact,
  ...
  return (
    <section style={{ marginBottom: '5rem' }} className={styles.contactSection}>
      ...
      {copyText.copied ? (
        <>
          <NotificationTile
            ...
            toggleTile={() => {
              setCopyText({ ...copyText, copied: !copyText.copied })
            }}
            tileText={`Copied Text: ${copyText.text}`}
          />
        </>
      ) : null}
      <div
        style={service.modalOpen ? { display: 'flex' } : { display: 'none' }}
        className={styles.phoneModal}
      >
        <div>
          <input
            style={{ width: 'fitContent', fontSize: '1.8rem' }}
            readOnly
            value="+91 9620062353"
            ref={copyRef}
          />
      ...
      <form id="form" onSubmit={submitHandler}>
        ...
        <label>
          Message
          <textarea onChange={contactChangeHandler} value={contact.message} name="message" />
        </label>
